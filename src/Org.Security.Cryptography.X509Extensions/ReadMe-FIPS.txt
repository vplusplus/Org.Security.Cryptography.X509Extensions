...
a) the provider only supports the TLS protocol as a client computer and as a server, if applicable. 
b) It uses only the Triple Data Encryption Standard (3DES) encryption algorithm for the TLS traffic encryption, 
c) only the Rivest-Shamir-Adleman (RSA) public key algorithm for the TLS key exchange and authentication, 
d) and only the Secure Hash Algorithm version 1 (SHA-1) hashing algorithm for the TLS hashing requirements.
</quote>

Also see:
https://docs.microsoft.com/en-us/archive/blogs/secguide/why-were-not-recommending-fips-mode-anymore
